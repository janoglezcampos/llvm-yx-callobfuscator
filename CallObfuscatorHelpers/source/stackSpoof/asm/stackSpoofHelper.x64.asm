...
;   Initialize spoof info if not already initialized
    ...
    fastcall_win64 __callobf_initializeSpoofInfo, p_info ;TODO: Check for error
    mov rcx, rbx
alreadyInitialized:
...
; For every list, pick a byte, put the value in range of each list entry count, and get the entry
    ...
    shl entry64, 4 ; Multiply by 16 (sizeof(FRAME_TABLE_ENTRY))
    ...
    mod entry32, DWORD [fieldPtr(STACK_SPOOF_INFO, p_info, pushRbpCount)]

    ; Since size of PUSH_RBP_FRAME_TABLE_ENTRY is 24 bytes, means rax * 24
    mov r11, entry64 ; tmp holder

    ; Multiply by 24 (sizeof(PUSH_RBP_FRAME_TABLE_ENTRY))
    ; rax * 24 = 8 * ((rax * 2) + rax)
    shl entry64, 1      ; (rax * 2)
    add entry64, r11    ; + "rax"
    shl entry64, 3      ; rax * 8

    lea  p_saveRbpFrameInfo, [fieldPtr(STACK_SPOOF_INFO, p_info, pushRbpList) + entry64]

;   Calculate the rbp value that will be "saved" by pushRbpFrame
    mov desiredRbpValue, [fieldPtr(STACK_SPOOF_INFO, p_info, p_entryRetAddr)]
    sub desiredRbpValue, [fieldPtr(FRAME_TABLE_ENTRY, p_setFpRegFrameInfo, frameSize)]

;   Set startting frame, used to set an arbitrary rsp value, based on rbp
    mov  rax, [fieldPtr(FRAME_TABLE_ENTRY, p_setFpRegFrameInfo, p_entryAddr)]
    checkNull rax, error
    push rax

;   Set frame to load arbitrary rbp value
    sub  rsp, [fieldPtr(PUSH_RBP_FRAME_TABLE_ENTRY, p_saveRbpFrameInfo, frameSize)]
    mov  rax, [fieldPtr(PUSH_RBP_FRAME_TABLE_ENTRY, p_saveRbpFrameInfo, p_entryAddr)]
    checkNull rax, error
    push rax

;   Store desiredRbpValue at the address where it should have been saved ?¿ <This sounds horrible>
    mov rax,  [fieldPtr(PUSH_RBP_FRAME_TABLE_ENTRY, p_saveRbpFrameInfo, rbpOffset)]
    add rax, 0x8
...
;   Set frame to redirect execution to the restore routine, it will need to be stored in rbx before execution begins, caller job
    sub  rsp, [fieldPtr(FRAME_TABLE_ENTRY, p_jmpRbxFrameInfo, frameSize)]
    mov  rax, [fieldPtr(FRAME_TABLE_ENTRY, p_jmpRbxFrameInfo, p_entryAddr)]
    checkNull rax, error
    push rax

;   Set frame to add some stack space so we can store args, shadow space, etc.
    sub  rsp, [fieldPtr(FRAME_TABLE_ENTRY, p_addRspFrameInfo, frameSize)]
    mov  rax, [fieldPtr(FRAME_TABLE_ENTRY, p_addRspFrameInfo, p_entryAddr)]
    checkNull rax, error
    push rax
